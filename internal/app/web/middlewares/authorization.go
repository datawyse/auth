...
func Authorization(log *zap.Logger, config *internal.AppConfig) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if strings.Contains(ctx.Request.URL.Path, "/auth/user/login") || strings.Contains(ctx.Request.URL.Path, "/auth/user/signup") || strings.Contains(ctx.Request.URL.Path, "/health") {
			ctx.Next()
			return
		}
